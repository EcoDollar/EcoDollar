...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of ecodollar addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) ecodollar address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"address\" : \"address\",  (string) the ecodollar address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ECODOLLAR address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the ecodollar address, the value is the ecodollar amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) ecodollar address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zECOS coinstakes (without MN output). Only for regtest\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zECOS is currently disabled due to maintenance.");
    ...
    // create the zerocoinmint output (one spent denom + three 1-zECOS denom)
    ...
    if (!pwalletMain->CreateZECOSOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zecos output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZECOSOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zecos output");
    }
    ...
    CTxIn newTxIn;
    // Use v2 spends for input (with v3 zecos staking is disabled)
...
UniValue createrawzerocoinspend(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "\nCreates raw zECOS public spend.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && !isPublicSpend)
        throw JSONRPCError(RPC_WALLET_ERROR, "zECOS old spend only available in regtest for tests purposes");
