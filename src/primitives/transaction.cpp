...
{
    return strprintf("CTxOut(nValue=%d.%07d, scriptPubKey=%s)", nValue / COIN, nValue % COIN, scriptPubKey.ToString().substr(0,30));
}
...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between ecos and zecos
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // ECODOLLAR: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
