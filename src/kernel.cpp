...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s : nStakeModifier=%s (nStakeModifierHeight=%s)\n"
                "nTimeBlockFrom=%d\nssUniqueIDD=%s\n-->DATA=%s",
            __func__, HexStr(modifier_ss), ((stake->IsZECOS()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())),
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zECOS stake block is less than allowed start height", __func__);

    if (CZEcosStake* zECOS = dynamic_cast<CZEcosStake*>(stake)) {
        CBlockIndex* pindexFrom = zECOS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zECOS stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zECOS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zECOS->GetValue()));
        if (nChecksum200 != zECOS->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zECOS->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZEcosStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zECOS fails context checks", __func__);
    } else {
        ...
        CEcosStake* ecosInput = new CEcosStake();
        ecosInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(ecosInput);
    }
    return true;
}
