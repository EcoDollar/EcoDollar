// Copyright (c) 2018-2020 The ECODOLLAR developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zecos/deterministicmint.h>
#include "zecostracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzECOSTracker::CzECOSTracker(std::string strWalletFile)
{
...
CzECOSTracker::~CzECOSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzECOSTracker::Init()
{
...
bool CzECOSTracker::Archive(CMintMeta& meta)
{
...
bool CzECOSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzECOSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzECOSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzECOSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzECOSTracker::GetSerialHashes()
{
...
CAmount CzECOSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzECOSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzECOSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzECOSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzECOSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzECOSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzECOSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzECOSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzECOSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzECOSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzECOSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzECOSWallet* zECOSWallet)
{
    bool iszECOSWalletInitialized = (NULL != zECOSWallet);
    CMintMeta meta;
    ...
    if (! iszECOSWalletInitialized)
        zECOSWallet = new CzECOSWallet(strWalletFile);
    meta.isSeedCorrect = zECOSWallet->CheckSeed(dMint);
    if (! iszECOSWalletInitialized)
        delete zECOSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzECOSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzECOSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzECOSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzECOSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzECOSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzECOSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzECOSWallet* zECOSWallet = new CzECOSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zECOSWallet);
        }
        delete zECOSWallet;
        LogPrint("zero", "%s: added %d dzecos from DB\n", __func__, listDeterministicDB.size());
    }
