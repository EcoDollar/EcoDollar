...
CzECOSWallet::CzECOSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zecos seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZECOSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZECOSSeed_deprecated()) {
                    LogPrintf("%s: Updated zECOS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zecos seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zecos wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzECOSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZECOSCount(nCountLastUsed);
    else if (!walletdb.ReadZECOSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzECOSWallet::Lock()
{
    seedMaster = 0;
}

void CzECOSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzECOSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZECOS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzECOSWallet::LoadMintPoolFromDB()
{
...
void CzECOSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzECOSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzECOSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zecosTracker->HasPubcoinHash(pMint.first)) {
...
bool CzECOSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZECOS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zecosTracker which also adds to database
    pwalletMain->zecosTracker->Add(dMint, true);
...
void CzECOSWallet::SeedToZECOS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzECOSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzECOSWallet::GenerateDeterministicZECOS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzECOSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzECOSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzECOSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
