...
#include "ui_zecoscontroldialog.h"
...
std::set<std::string> ZEcosControlDialog::setSelectedMints;
std::set<CMintMeta> ZEcosControlDialog::setMints;

bool CZEcosControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEcosControlDialog::COLUMN_DENOMINATION || column == ZEcosControlDialog::COLUMN_VERSION || column == ZEcosControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEcosControlDialog::ZEcosControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEcosControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zECOS Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZEcos->setProperty("cssClass", "text-main-purple");
    ui->labelZEcos_int->setProperty("cssClass", "text-main-purple");
...
ZEcosControlDialog::~ZEcosControlDialog()
{
    delete ui;
}

void ZEcosControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEcosControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEcosControlWidgetItem* itemDenom(new CZEcosControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEcosControlWidgetItem *itemMint = new CZEcosControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zECOS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zECOS seed used to mint this zECOS is not the same as currently hold in the wallet";
            else
...
void ZEcosControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEcosControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEcos_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZEcosControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEcosControlDialog::GetSelectedMints()
{
...
void ZEcosControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
