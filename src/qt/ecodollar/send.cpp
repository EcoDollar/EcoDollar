...
#include "qt/ecodollar/send.h"
#include "qt/ecodollar/forms/ui_send.h"
#include "qt/ecodollar/addnewcontactdialog.h"
#include "qt/ecodollar/qtutils.h"
#include "qt/ecodollar/sendchangeaddressdialog.h"
#include "qt/ecodollar/optionbutton.h"
#include "qt/ecodollar/sendconfirmdialog.h"
#include "qt/ecodollar/myaddressrow.h"
#include "qt/ecodollar/guitransactionsutils.h"
...
#include "zecoscontroldialog.h"

SendWidget::SendWidget(ECODOLLARGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("ECOS");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zECOS");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ECOS) or private coins (zECOS)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("ECODOLLAR address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 ECOS");
    ...
    setCssProperty(coinIcon, "coin-icon-ecos");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onECOSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onECOSSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send ECOS");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zECOS");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZecos = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZecos));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZecos ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendEcos = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendEcos ? AskPassphraseDialog::Context::Send_ECOS : AskPassphraseDialog::Context::Send_zECOS, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendEcos) ? send(recipients) : sendZecos(recipients)) {
...
bool SendWidget::sendZecos(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zECOS is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zECOS selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZEcosControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZEcosControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZecos(
        ...
        inform(tr("zECOS transaction sent!"));
        ZEcosControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZECOS_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zECOS require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zECOS transaction
        ...
        emit message("zECOS transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isECOS){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any ECOS to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZEcosControlDialog *zEcosControl = new ZEcosControlDialog(this);
            zEcosControl->setModel(walletModel);
            zEcosControl->exec();
            ui->btnCoinControl->setActive(!ZEcosControlDialog::setSelectedMints.empty());
            zEcosControl->deleteLater();
        } else {
            inform(tr("You don't have any zECOS in your balance to select."));
        }
    }
}
...
void SendWidget::onECOSSelected(bool _isECOS){
    isECOS = _isECOS;
    setCssProperty(coinIcon, _isECOS ? "coin-icon-ecos" : "coin-icon-zecos");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress ecosAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(ecosAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ecosAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
