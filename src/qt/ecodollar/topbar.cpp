...
#include "qt/ecodollar/topbar.h"
#include "qt/ecodollar/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/ecodollar/lockunlock.h"
#include "qt/ecodollar/qtutils.h"
#include "qt/ecodollar/receivedialog.h"
...
TopBar::TopBar(ECODOLLARGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopEcos, ui->labelAmountTopzEcos}, "amount-small-topbar");
    setCssProperty({ui->labelAmountEcos, ui->labelAmountzEcos}, "amount-topbar");
    setCssProperty({ui->labelPendingEcos, ui->labelPendingzEcos, ui->labelImmatureEcos, ui->labelImmaturezEcos}, "amount-small-topbar");

    ui->labelAmountTopzEcos->setVisible(false);
    ui->label_16->setVisible(false);
    ui->labelTitle2->setVisible(false);
    ui->labelAmountzEcos->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzEcos->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezEcos->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("ECODOLLAR")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // ECOS Total
    CAmount ecosAvailableBalance = balance;
    // zECOS Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalEcos = GUIUtil::formatBalance(ecosAvailableBalance, nDisplayUnit);
    QString totalzEcos = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopEcos->setText(totalEcos);
    ui->labelAmountTopzEcos->setText(totalzEcos);

    // Expanded
    ui->labelAmountEcos->setText(totalEcos);
    ui->labelAmountzEcos->setText(totalzEcos);

    ui->labelPendingEcos->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzEcos->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureEcos->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezEcos->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
