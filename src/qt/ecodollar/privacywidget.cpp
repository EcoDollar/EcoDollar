...
#include "qt/ecodollar/privacywidget.h"
#include "qt/ecodollar/forms/ui_privacywidget.h"
#include "qt/ecodollar/qtutils.h"
#include "guiutil.h"
#include "qt/ecodollar/denomgenerationdialog.h"
#include "qt/ecodollar/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(ECODOLLARGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zECOS anonymizes your ECOS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zECOS or convert back to ECOS"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 ECOS ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zECOS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zECOS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zECOS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zECOS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zECOS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zECOS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zECOS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zECOS");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zECOS Movements"));
    ...
    // Only Convert to ECOS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzECOS->setTitleClassAndText("btn-title-grey", "Total 0 zECOS");
    ui->btnTotalzECOS->setSubTitleClassAndText("text-subtitle", "Show denominations of zECOS owned.");
    ui->btnTotalzECOS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ECOS outputs to mint into zECOS.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zECOS");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzECOS, SIGNAL(clicked()), this, SLOT(onTotalZecosClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zECOS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of ECOS to mint into zECOS"));
    }else{
        btnText = tr("Convert back to ECOS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zECOS to convert back into ECOS"));
    }
...
void PrivacyWidget::onTotalZecosClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzECOS->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzECOS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zECOS is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zECOS, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zECOS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zECOS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZecos(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zECOS converted back to ECOS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zECOS </b>";
    ...
    ui->btnTotalzECOS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
