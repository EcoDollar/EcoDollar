...
#include "qt/ecodollar/dashboardwidget.h"
#include "qt/ecodollar/forms/ui_dashboardwidget.h"
#include "qt/ecodollar/sendconfirmdialog.h"
#include "qt/ecodollar/txrow.h"
...
DashboardWidget::DashboardWidget(ECODOLLARGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ECOS staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareEcos, "square-chart-ecos");
    setCssProperty(ui->labelSquarezEcos, "square-chart-zecos");
    ...
    ui->labelAmountZecos->setText("0 zECOS");
    ui->labelAmountEcos->setText("0 ECOS");
    setCssProperty(ui->labelAmountEcos, "text-stake-ecos-disable");
    setCssProperty(ui->labelAmountZecos, "text-stake-zecos-disable");

    ui->labelAmountZecos->setVisible(false);
    ...
    ui->btnHowTo->setText(tr("How to get ECOS"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("ECOS")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isEcos = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZECOS;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair ECOS, zECOS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ecos = (pair.first != 0) ? pair.first / 10000000 : 0;
            zecos = (pair.second != 0) ? pair.second / 10000000 : 0;
            chartData->totalEcos += pair.first;
            chartData->totalZecos += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEcos.append(ecos);
        chartData->valueszEcos.append(zecos);

        int max = std::max(ecos, zecos);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("ECOS");
    set1 = new QBarSet("zECOS");
    ...
    set0->append(chartData->valuesEcos);
    set1->append(chartData->valueszEcos);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEcos > 0 || chartData->totalZecos > 0) {
        setCssProperty(ui->labelAmountEcos, "text-stake-ecos");
        setCssProperty(ui->labelAmountZecos, "text-stake-zecos");
    } else {
        setCssProperty(ui->labelAmountEcos, "text-stake-ecos-disable");
        setCssProperty(ui->labelAmountZecos, "text-stake-zecos-disable");
    }
    forceUpdateStyle({ui->labelAmountEcos, ui->labelAmountZecos});
    ui->labelAmountEcos->setText(GUIUtil::formatBalance(chartData->totalEcos, nDisplayUnit));
    ui->labelAmountZecos->setText(GUIUtil::formatBalance(chartData->totalZecos, nDisplayUnit, true));
