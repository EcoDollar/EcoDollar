...
#include "qt/ecodollar/addresseswidget.h"
#include "qt/ecodollar/forms/ui_addresseswidget.h"
#include "qt/ecodollar/addresslabelrow.h"
#include "qt/ecodollar/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(ECODOLLARGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    ui->btnAddContact->setSubTitleClassAndText("text-subtitle", "Generate a new address to receive EcoDollar coins.");
    ...
    // Address
    ui->labelAddress->setText(tr("Enter ECODOLLAR address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress ecosAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(ecosAdd.Get(), label.toUtf8().constData(),
                ecosAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
