...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ecodollar address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ECOS.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ecodollar:URI (ecodollar:XyZ...?message=example)
            {
...
bool WalletModel::createZecosSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZecos(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzECOSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzECOSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzECOSReset.connect(boost::bind(NotifyzECOSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzECOSReset.disconnect(boost::bind(NotifyzECOSReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid ECODOLLAR address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid ECODOLLAR address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from ECODOLLAR address");
...
{
    setMints.clear();
    setMints = pwalletMain->zecosTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
