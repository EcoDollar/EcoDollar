...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZECOSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZECOSStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZECOSStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZECOSStake);
    CAmount backboneValue = GetBackbonePayment(pindexPrev->nHeight, blockValue);
        ...
        if (fProofOfStake) {
            ...
            if (!txNew.vout[1].IsZerocoinMint()) {
                if (i == 2) {
                    ...
                    txNew.vout[i - 1].nValue -= (masternodePayment + backboneValue);
                } else if (i > 2) {
                    // special case, stake is split between (i-1) outputs
                    unsigned int outputs = i - 1;
                    CAmount total_payment = masternodePayment + backboneValue;
            ...
            txNew.vout[i + 1].nValue = backboneValue; 
            ...
            txNew.vout[2].nValue = backboneValue;
            txNew.vout[0].nValue = blockValue - masternodePayment - backboneValue;
        }
